...
public class Product
{
   ...
   private string NormalizeDescription(string description)
   {
      if (description.Trim().Length > 50)
         throw new ArgumentException("The description exceeded 50 characters.");
      return description.Trim().ToLower();
   }
   private decimal NormalizePrice(decimal price)
   {
      if (decimal.IsNegative(price) && price < 0)
         throw new ArgumentException("Invalid price - dont set negative number");
      return price;
   }
